...
import { Compass, MapPin, Search, Flame, X, Store, User, ArrowLeftRight, Package, ChevronRight, Calendar, Menu, Navigation } from 'lucide-react'
import { useAuth } from './hooks/useAuth'
import { useShops, useReviews, useTradePosts, useVault } from './hooks/useShops'
import { startCheckout } from './lib/stripe'

type TabType = 'discover' | 'map' | 'classifieds' | 'vault' | 'profile'
type ModalType = 'none' | 'sub' | 'auth' | 'ar' | 'shop' | 'menu'

// ── Tiny map built with SVG — no API key needed ──────────────────────────────
    ...
    <div className="relative w-full rounded-3xl overflow-hidden border-2 border-white/20 shadow-2xl" style={{ background: '#1a1f2e' }}>
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} className="block">
        ...
        {shops.map((s, i) => {
          ...
          const color = s.category === 'comics' ? '#F59E0B' : '#38BDF8'
          return (
      ...
      {/* Legend */}
      <div className="absolute bottom-3 left-3 flex gap-3 text-[9px] font-bold">
      ...
      <div className="absolute top-3 right-3 text-[8px] font-mono text-white/30 font-bold">DENVER METRO</div>
    </div>
  )
}
...
export default function App() {
  ...
  return (
    <div className="min-h-screen text-[#18191B] flex flex-col font-sans max-w-md mx-auto relative" style={{ background: '#F0EFE9' }}>
      ...
      <header className="sticky top-0 z-20 px-4 pt-12 pb-3" style={{ background: 'linear-gradient(135deg, #1a0a2e 0%, #16213e 100%)' }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-2xl flex items-center justify-center shadow-lg" style={{ background: 'linear-gradient(135deg, #E0533C, #ff6b4a)' }}>
          ...
          <div className="flex items-center gap-2">
            {isSignedIn && (
              <div className="px-3 py-1.5 rounded-xl text-[10px] font-bold" style={{ background: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.7)' }}>
                @{profile?.username}
              </div>
            )}
      ...
      <main className="flex-1 overflow-y-auto pb-28">
            ...
            <div className="flex gap-2 pt-1">
              {[
                ...
                <button key={f.id} onClick={() => setFilter(f.id)}
                  className="px-4 py-2 rounded-2xl text-xs font-black uppercase border-2 transition-all"
                  style={filter === f.id
            ...
            {shops[0] && (
              <div className="rounded-3xl p-4 text-white relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1a0a2e, #302b63)' }}>
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-orange-400">Latest Drop</span>
                </div>
                <p className="text-sm font-bold leading-snug opacity-90">"{shops[0].hot_find}"</p>
                <p className="text-[10px] mt-1 opacity-40 font-mono">{shops[0].name}</p>
              </div>
            )}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-1.5 mt-2.5 flex-wrap">
                      {s.tags?.map((t: string, i: number) => (
                        <span key={i} className="text-[10px] bg-zinc-100 text-zinc-600 px-2 py-1 rounded-lg font-bold">{t}</span>
                      ))}
                    </div>
                  </div>
                  <div className="h-10 w-10 rounded-2xl flex items-center justify-center flex-shrink-0"
                    style={{ background: s.category === 'comics' ? '#FEF3C7' : '#E0F2FE' }}>
                    <MapPin className="h-5 w-5" style={{ color: s.category === 'comics' ? '#D97706' : '#0284C7' }} />
                  </div>
                </div>
                {s.hot_find && (
        ...
        {tab === 'map' && (
          <div className="p-4 space-y-4">
            <div className="rounded-3xl p-4 text-white" style={{ background: 'linear-gradient(135deg, #1a0a2e, #302b63)' }}>
              ...
              <p className="text-xs text-white/40 mt-0.5">Denver Metro Area</p>
            </div>
          ...
          <div className="p-4 space-y-4">
            ...
            {tradePosts.map((p: any) => (
              <div key={p.id} className="bg-white rounded-3xl p-4 shadow-sm border border-zinc-100">
                <p className="text-[10px] font-mono text-zinc-400 mb-3">@{p.username}</p>
                <div className="space-y-2">
                  <div className="flex gap-2 items-center">
                    <span className="text-[10px] font-black px-2 py-1 rounded-lg uppercase" style={{ background: '#F0FDF4', color: '#166534' }}>OFFER</span>
                    <p className="text-sm font-bold">{p.offer}</p>
                  </div>
                  <div className="flex gap-2 items-center">
                    <span className="text-[10px] font-black px-2 py-1 rounded-lg uppercase" style={{ background: '#FEF2F2', color: '#991B1B' }}>WANT</span>
        ...
        {tab === 'vault' && (
          <div className="p-4 space-y-4">
            ...
            <div className="bg-white rounded-3xl p-4 shadow-sm border border-zinc-100">
              <h3 className="font-black text-sm mb-3">Add Item</h3>
              <form onSubmit={handleVaultSubmit} className="space-y-3">
                ...
                <button type="submit" disabled={!isSignedIn}
                  className="w-full text-white font-black py-3.5 rounded-2xl text-sm uppercase tracking-wide disabled:opacity-40"
                  style={{ background: isSignedIn ? 'linear-gradient(135deg, #1a0a2e, #302b63)' : '#ccc' }}>
            ...
            <div className="space-y-2">
              {vaultItems.map((item: any) => (
                <div key={item.id} className="bg-white rounded-2xl p-4 flex justify-between items-center shadow-sm border border-zinc-100">
                  <p className="font-bold text-sm">{item.name}</p>
                  <p className="font-black text-base" style={{ color: '#059669' }}>${item.est_value.toLocaleString()}</p>
                </div>
              ))}
              {vaultItems.length === 0 && (
      ...
      {/* ── BOTTOM NAV ── */}
      <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md border-t border-zinc-200 px-2 py-3 pb-7 flex items-center justify-around z-20"
        ...
        {[
          ...
          <button key={id} onClick={() => setTab(id as TabType)}
            ...
            <div className="h-10 w-10 rounded-2xl flex items-center justify-center transition-all"
              style={tab === id ? { background: 'linear-gradient(135deg, #E0533C, #ff6b4a)' } : { background: 'transparent' }}>
              <Icon className="h-5 w-5" style={{ color: tab === id ? 'white' : '#9ca3af' }} />
            </div>
      ...
      {modal === 'shop' && selectedShop && (
        <div className="fixed inset-0 z-30 flex flex-col overflow-hidden" style={{ background: '#F0EFE9' }}>
          <div className="px-4 pt-12 pb-4 flex items-center gap-3" style={{ background: 'linear-gradient(135deg, #1a0a2e, #302b63)' }}>
            <button onClick={() => setModal('none')} className="h-9 w-9 rounded-2xl flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.15)' }}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <div className="bg-white rounded-3xl p-4 shadow-sm border border-zinc-100">
              <span className="text-xs font-black uppercase px-2.5 py-1 rounded-xl"
                style={(selectedShop as any).category === 'comics'
                  ? { background: '#FEF3C7', color: '#92400E' }
                  : { background: '#E0F2FE', color: '#0369A1' }}>
            ...
            <div className="rounded-3xl p-4 text-white relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1a0a2e, #302b63)' }}>
              <div className="flex items-center gap-2 mb-2">
                <Flame className="h-4 w-4 text-orange-400" />
                <span className="text-[10px] font-black uppercase tracking-widest text-orange-400">Live Floor Drop</span>
              </div>
              ...
              {isMerchant && (selectedShop as any).owner_id === user?.id && (
                <form onSubmit={async e => { e.preventDefault(); if (!inpFind.trim()) return; await updateHotFind(selectedShop.id, inpFind); setInpFind('') }}
                  ...
                  <input type="text" value={inpFind} onChange={e => setInpFind(e.target.value)}
                    ...
                    className="w-full rounded-2xl px-4 py-3 text-sm font-medium outline-none text-white placeholder:text-white/30"
                    style={{ background: 'rgba(255,255,255,0.1)' }} />
                  <button type="submit" className="w-full py-2.5 rounded-2xl text-sm font-black uppercase" style={{ background: 'rgba(255,255,255,0.2)', color: 'white' }}>Publish</button>
                </form>
              )}
            </div>
              ...
              <div className="bg-white rounded-3xl p-4 shadow-sm border border-zinc-100">
                ...
                <div className="space-y-2">
                  {(selectedShop as any).events.map((ev: any) => (
                    <div key={ev.id} className="flex items-center justify-between p-3 rounded-2xl" style={{ background: '#F8F7F2' }}>
                      <div>
                        <span className="text-[10px] bg-zinc-200 px-2 py-0.5 rounded-lg font-mono font-bold mr-2">{ev.date}</span>
            ...
            <div className="bg-white rounded-3xl p-4 shadow-sm border border-zinc-100">
              ...
              <div className="space-y-2 mb-3">
                {reviews.map((r: any) => (
                  <div key={r.id} className="p-3 rounded-2xl" style={{ background: '#F8F7F2' }}>
                    <p className="text-sm font-medium">"{r.comment}"</p>
                    <p className="text-[10px] font-mono font-bold mt-1" style={{ color: '#E0533C' }}>@{r.username}</p>
                  </div>
                ))}
                {reviews.length === 0 && <p className="text-sm text-zinc-400 italic">No reviews yet</p>}
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-5">
                    {[
                      ...
                      <button key={r.id} onClick={() => setRole(r.id as any)}
                        ...
                        <span className="text-[9px] opacity-50 font-mono">{r.sub}</span>
                      </button>
                    ))}
                  </div>
                    ...
                    <div className="flex gap-2 justify-center">
                      {authCode.map((digit, i) => (
                        <input key={i} ref={codeRefs[i]} type="text" inputMode="numeric" maxLength={1} value={digit}
                          ...
                          className="w-10 h-13 text-center text-lg font-black border-2 rounded-2xl outline-none transition-all bg-white"
                          style={{ borderColor: digit ? '#1a0a2e' : '#e5e7eb', caretColor: 'transparent', height: '3rem' }} />
                      ))}
                    </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end justify-center">
          <div className="w-full max-w-md rounded-t-3xl p-5 pb-10 shadow-2xl" style={{ background: '#FAF9F5' }}>
            <div className="flex justify-between items-center mb-5">
              <h3 className="font-black text-lg">Membership</h3>
              <button onClick={() => setModal('none')}><X className="h-5 w-5 text-zinc-400" /></button>
            </div>
            <div className="space-y-3">
              {[
                { name: 'Hunter Base', sub: 'Basic access', price: 'Free', tier: 'free', style: { border: '2px solid #e5e7eb', background: 'white' }, btn: { background: '#f4f4f5', color: '#71717a' }, btnLabel: 'Current' },
                { name: 'Elite Pass', sub: 'Price alerts · AR scan', price: '$1.99/mo', tier: 'elite', style: { border: '2px solid #E0533C', background: 'white' }, btn: { background: 'linear-gradient(135deg, #E0533C, #ff6b4a)', color: 'white' }, btnLabel: 'Upgrade' },
                { name: 'Verified Store', sub: 'Merchant dashboard', price: '$2.99/mo', tier: 'store', style: { background: 'linear-gradient(135deg, #1a0a2e, #302b63)' }, btn: { background: '#F59E0B', color: 'white' }, btnLabel: 'Claim Slot', dark: true },
              ].map(item => (
                <div key={item.tier} className="rounded-3xl p-4 flex items-center justify-between" style={item.style as any}>
                  <div>
                    <p className="font-black text-sm" style={{ color: item.dark ? 'white' : 'inherit' }}>{item.name}</p>
                    <p className="text-xs mt-0.5" style={{ color: item.dark ? 'rgba(255,255,255,0.4)' : '#9ca3af' }}>{item.sub}</p>
                    <p className="font-black text-lg mt-1" style={{ color: item.dark ? 'white' : 'inherit' }}>{item.price}</p>
                  </div>
                  <button onClick={() => item.tier !== 'free' ? handleUpgrade(item.tier as any) : setModal('none')}
                    disabled={checkoutLoading}
                    className="font-black text-xs uppercase px-4 py-2.5 rounded-2xl disabled:opacity-50"
                    style={item.btn as any}>
                    {checkoutLoading ? '...' : item.btnLabel}
                  </button>
                </div>
              ))}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex flex-col text-white font-mono" style={{ background: '#0a0a0f' }}>
          ...
          <div className="flex-1 relative flex items-center justify-center p-8"
            ...
            <div className="p-5 rounded-3xl text-left w-full max-w-xs" style={{ background: '#111827', border: '1px solid #F59E0B' }}>
              <span className="text-[10px] text-amber-400 font-black uppercase tracking-widest">★ Wishlist Match</span>
